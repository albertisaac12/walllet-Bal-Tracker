...
//       process.env.BALANCE_FETCHER_API
//     );

//     // Load wallets and channels from JSON file
...
//     for (let wallet of adminWallets) {
...
//         }
//       } catch (walletError) {
//         console.error(`Error fetching balance for ${wallet}:`, walletError);
//       }
//     }
...
app.post("/slack/update", async (req, res) => {
  ...
  const [action, keyword, param1, param2] = text.split(" ").slice(1);
...
slackApp.message(async ({ message, say, context }) => {
  ...
  // Check if the message is exactly "hello" and contains a mention of the bot
  if (message.text.toLowerCase() === `hello <@${botUserId}>`) {
    await say(
